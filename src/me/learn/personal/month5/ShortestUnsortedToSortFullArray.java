 ...
 *
 */
public class ShortestUnsortedToSortFullArray {
...
c) Find the last element (if there is any) in arr[e+1..n-1] which is smaller than max, change e to index of this element. In the above example 1, e is changed to 8 (index of 35)
	...
	public int findUnsortedSubarray(int[] arr) {
		...
		// step 2(a) of above algo, find the min and max
		max = arr[s];
		min = arr[s];
		for (i = s + 1; i <= e; i++) {
			if (arr[i] > max)
		...
		// see if you have any element less than min
		...
		// see if you have an element greater then max
