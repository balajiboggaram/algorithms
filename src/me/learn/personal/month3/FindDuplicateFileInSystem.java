 ...
 * Note:
 * 
 * No order is required for the final output. You may assume the directory name,
 * file name and file content only has letters and digits, and the length of
 * file content is in the range of [1,50]. The number of files given is in the
 * range of [1,20000]. You may assume no files or directories share the same
...
public class FindDuplicateFileInSystem {
	...
	public static void main(String args[]) {
		...
		System.out.println(solution.findDuplicate(new String[] {"root/a 1.txt(abcd) 2.txt(efgh)","root/c 3.txt(abcd)"}));
	}

	// MAP of content (or md5sum) -> list of files 
	public List<List<String>> findDuplicate(String[] paths) {
		
		Map<String, List<String>> map = new HashMap<String, List<String>>();
		
		for(int i = 0;i< paths.length;i++) {
			String[] tokens = paths[i].split("\\s+");
			String dir = tokens[0];
			
			for(int j = 1; j < tokens.length; j++) {
				
				String content = tokens[j].split("\\(")[1]; // ensure you escape here.
				String fileName = tokens[j].split("\\(")[0];
								
				if(map.containsKey(content)) { // here you can also use checksum as key for larger files
	...
	// Just in case - how to compute the checksum MD5 or sha-1
	private String computeMD5Sum(String s) throws NoSuchAlgorithmException {
		
		MessageDigest md5Sum = MessageDigest.getInstance("MD5");
		
		byte[] digest = md5Sum.digest(s.getBytes());
		
		return new String(digest, StandardCharsets.UTF_16);
	}
